...
class PhaseNet:
    def __init__(self, **kwargs):
        """
        Initializes the model with dynamic arguments using argparse-style parsing.
        All arguments are collected through a parser-like structure, passed as kwargs.

        Args:
            **kwargs: Arbitrary keyword arguments that represent model configurations.
        """
        # Create a parser and parse the arguments
        args = self.get_args_parser(kwargs)

        # Initialize instance variables based on parsed args
        ...
        self.highpass_filter = args.highpass_filter
        self.response_path = args.response_path
        self.response_xml = args.response_xml
        self.nt = args.nt
        self.nx = args.nx
        self.resample_time = args.resample_time
        self.resample_space = args.resample_space
        ...
        self.use_deterministic_algorithms = args.use_deterministic_algorithms
        self.amp = args.amp
    ...
    def get_args_parser(kwargs):
        ...
        parser = argparse.ArgumentParser(description='EQNet Model', add_help=True)

        # model
        parser.add_argument(
            '--model', default='phasenet_das', type=str, help='model name'
        )
        parser.add_argument('--resume', default='', type=str, help='path of checkpoint')
        parser.add_argument(
            '--backbone', default='unet', type=str, help='model backbone'
        )
        parser.add_argument(
            '--phases', default=['P', 'S'], type=str, nargs='+', help='phases to use'
        )
        ...
        parser.add_argument(
            '--data_list', type=str, default=None, help='selectecd data list'
        )
        parser.add_argument(
            '--hdf5-file', default=None, type=str, help='hdf5 file for training'
        )
        parser.add_argument(
            '--prefix', default='', type=str, help='prefix for the file name'
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default='das',
            help='dataset type; seismic_trace, seismic_network, das',
        )
        parser.add_argument(
            '--result_path',
            type=str,
        ...
        parser.add_argument(
            ...
            default=0.3,
            type=float,
            help='minimum probability for picking',
        )

        ## Seismic
        parser.add_argument(
            '--add_polarity', action='store_true', help='If use polarity information'
        )
        parser.add_argument(
            '--add_event', action='store_true', help='If use event information'
        )
        parser.add_argument(
            '--sampling_rate',
            type=float,
            default=100.0,
            help='sampling rate; default 100.0 Hz',
        )
        parser.add_argument(
            '--highpass_filter',
            type=float,
            default=None,
            help='highpass filter; default 0.0 is no filter',
        )
        parser.add_argument(
            '--response_path', default=None, type=str, help='response path'
        )
        parser.add_argument(
            '--response_xml', default=None, type=str, help='response xml file'
        )
        parser.add_argument(
            '--subdir_level', default=0, type=int, help='folder depth for data list'
        )

        ## DAS
        parser.add_argument(
            '--cut_patch', action='store_true', help='If cut patch for continuous data'
        )
        parser.add_argument(
            '--nt',
            default=1024 * 20,
            type=int,
            help='number of time samples for each patch',
        )
        parser.add_argument(
            '--nx',
            default=1024 * 5,
            type=int,
            help='number of spatial samples for each patch',
        )
        parser.add_argument(
            '--resample_time',
            action='store_true',
            help='If resample time for continuous data',
        )
        parser.add_argument(
            '--resample_space',
            action='store_true',
            help='If resample space for continuous data',
        )
        parser.add_argument(
            '--system',
            type=str,
            default=None,
            help='The name of system of different system: optasense, eqnet, or None',
        )
        parser.add_argument(
            '--location', type=str, default=None, help='The name of systems at location'
        )
        parser.add_argument(
            '--skip_existing', action='store_true', help='Skip existing files'
        )
        # Create a list of command-line style arguments
        args = []
